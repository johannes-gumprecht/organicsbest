...
window.recentlyViewed={init(){let a;let b,c;if($(".js-product_section[data-rv-handle]").length){a=$(".js-product_section").data("rv-handle").toString();var d=window.Cookies.get("recentlyViewed");d=window.recentlyViewed.getCookieProducts(d,a)}else $(".recently-viewed__section").length?(d=window.Cookies.get("recentlyViewed"),d=window.recentlyViewed.getCookieProducts(d,a)):$(".js-sidebar-recently-viewed").length&&(d=window.Cookies.get("recentlyViewed"),d=window.recentlyViewed.getCookieProducts(d,a));
...
escape(a.join(",")),{expires:30,path:"/",sameSite:"None",secure:!0})},getProductInformation(a,b,c){$(a).attr("data-recently-viewed-items-loaded","true");c&&b.splice($.inArray(c,b),1);const d=$(a).data("visible-products");d&&b&&(b=b.slice(0,d));$.each(b,(f,e)=>{e&&($(a).removeClass("hidden"),$(a).parents(".sidebar-block").show(),$.ajax({type:"GET",url:`/products/${e}?view=rv`,success(g){g=$(g).find(".js-recently-viewed-product");$(a).find(` .rv-box-${f}`).append(g);window.PXUTheme.currency.show_multiple_currencies&&
window.currencyConverter.convertCurrencies();window.PXUTheme.theme_settings.collection_secondary_image&&window.imageFunctions.showSecondaryImage();0<g.find("[data-html5-video] video, [data-youtube-video]").length&&window.videoFeature.setupPlayerForRecentlyViewedProducts(g.find("[data-html5-video] video, [data-youtube-video]"))},error(g,h,k){console.log(g);console.log(h);console.log(k)},dataType:"html"}));$(a).find(".rv-main").hasClass("js-rv-slider")?(b.length<=d?$(".js-rv-slider .gallery-cell").eq(b.length).nextAll().addBack().remove():
$(".js-rv-slider .gallery-cell").eq(d).nextAll().addBack().remove(),window.recentlyViewed.createSlider(a,d)):$(a).find(".rv-main").hasClass("js-rv-grid")&&(b.length<=d?$(".js-rv-grid .thumbnail").eq(b.length).nextAll().addBack().remove():$(".js-rv-grid .thumbnail").eq(d).nextAll().addBack().remove())})},createSlider(){const a=$(".js-rv-slider").find(".gallery-cell").length,b=$(".js-rv-slider").data("products-available");let c="left",d=!0,f=!0,e;window.productsPerSlide>=a&&(c="center",f=d=!1);"2"===
window.productsPerSlide&&b>window.productsPerSlide||"4"===window.productsPerSlide&&b>window.productsPerSlide||"6"===window.productsPerSlide&&b>window.productsPerSlide?e=0:"3"===window.productsPerSlide&&b?e=1:"5"===window.productsPerSlide&&b?e=2:"7"===window.productsPerSlide&&b&&(e=3);$(".js-rv-slider").flickity({lazyLoad:2,imagesLoaded:!0,prevNextButtons:f,wrapAround:d,cellAlign:c,pageDots:window.usePageDots,contain:!0,freeScroll:!0,arrowShape:window.arrowSize,initialIndex:e});$(".js-rv-slider").addClass("slider-initialized")}};
var enableLoadMoreButton=function(a){function b(f,e){$.ajax({type:"GET",dataType:"html",url:f,beforeSend:function(){e.addClass("is-loading");$(".load-more__icon").addClass("loading-in-progress").css({height:"40px",width:"40px",opacity:"1"})},success:function(g){e.removeClass("is-loading");$(".load-more__icon").removeClass("loading-in-progress").css({height:"0",width:"0",opacity:"0"});var h=$(g).find(d);g=$(g).find("[data-load-more]").attr("href");$("[data-load-more]").attr("href",g);$(c).first().append(h).append($(".js-load-more"));
"undefined"==typeof g&&$("[data-load-more]").addClass("is-hidden")},error:function(g,h,k){console.log(g);console.log(h);console.log(k);location.replace(location.protocol+"//"+location.host+filterURL)}})}var c=a+"[data-load-more--grid]",d=a+" [data-load-more--grid-item]";$("body").on("click","[data-load-more]",function(f){f.preventDefault();f=$(this);var e=f.attr("href");b(e,f)})};
window.enableLoadMoreProducts=function(){$("body").on("click",".js-load-more a",function(a){enableInfiniteScroll();a.stopPropagation();return!1})};
...
SPR.initRatingHandler(),SPR.initDomEls(),SPR.loadProducts(),SPR.loadBadges()});window.PXUTheme.currency.show_multiple_currencies&&window.currencyConverter.convertCurrencies();Shopify.PaymentButton&&Shopify.PaymentButton.init();window.productPage.init();window.productPage.runOptionSelector();window.PXUTheme.theme_settings.quick_shop_enabled&&window.quickShop.init();window.hideNoScript();window.videoFeature.init();window.PXUTheme.theme_settings.collection_secondary_image&&window.imageFunctions.showSecondaryImage();
Waypoint.refreshAll()}})},enableLoadMoreSearch=function(){$("body").on("click",".js-load-more a",function(a){enableInfiniteSearchScroll();a.stopPropagation();return!1})},enableInfiniteSearchScroll=function(){$(".search-matrix").length&&new Waypoint.Infinite({element:$(".search-matrix")[0],items:".search-matrix",more:"[data-load-more]",loadingClass:"loading-in-progress",onBeforePageLoad:function(){$(".js-load-more").hide()},onAfterPageLoad:function(a){$(".search-matrix > .search-matrix .thumbnail").unwrap(".search-matrix");
window.PXUTheme.theme_settings.enable_shopify_review_comments&&SPR.$(document).ready(function(){return SPR.registerCallbacks(),SPR.initRatingHandler(),SPR.initDomEls(),SPR.loadProducts(),SPR.loadBadges()});window.PXUTheme.currency.show_multiple_currencies&&window.currencyConverter.convertCurrencies();Shopify.PaymentButton&&Shopify.PaymentButton.init();window.productPage.init();window.productPage.runOptionSelector();window.PXUTheme.theme_settings.quick_shop_enabled&&window.quickShop.init();window.hideNoScript();
window.videoFeature.init();window.PXUTheme.theme_settings.collection_secondary_image&&window.imageFunctions.showSecondaryImage()}})};window.hideNoScript=function(){$(".image__container .noscript").addClass("hidden")};Shopify.queryParams={};if(location.search.length)for(var aKeyValue,i=0,aCouples=location.search.substr(1).split("&");i<aCouples.length;i++)aKeyValue=aCouples[i].split("="),1<aKeyValue.length&&(Shopify.queryParams[decodeURIComponent(aKeyValue[0])]=decodeURIComponent(aKeyValue[1]));
var quickFilter={init:function(){var a=[],b="",c="";b=$.url("1")?"/"+$.url("1")+"/":"";4>=b.length?(url2=$.url("2")?$.url("2")+"/":"",url3=$.url("3")?$.url("3")+"/":"",path=b+url2+url3):(url2=$.url("2")?$.url("2")+"/":"",path=b+url2);b=$("#sort-by").length?$("#sort-by").val():url("?sort_by");if($("#tag_filter").length){var d=$("#tag_filter").val();$("#tag_filter").data("default-collection")!=d&&(urlTag=d.substr(d.lastIndexOf("/")+1),d=d.substr(d),$("#tag_filter option").removeAttr("selected"),$('#tag_filter option[value="'+
d+'"]').attr("selected","selected"),"all"==urlTag||-1<$.inArray(urlTag,a)||a.unshift(urlTag))}$("[data-option-filter] input:checked").each(function(){a.push($(this).val())});a=$.makeArray(a);$.each(a,function(f,e){c=f!=a.length-1?c+(a[f]+"+"):c+a[f]});Shopify.queryParams.sort_by=b;b="?"+$.param(Shopify.queryParams);quickFilter.processUrl(path,c,b)},updateView:function(a){$.ajax({type:"GET",url:a,beforeSend:function(){$(".collection-matrix").addClass("fadeOut animated loading-in-progress filter-loading");
Waypoint.destroyAll()},success:function(b){$(".collection-matrix").removeClass("loading-in-progress");$(".collection-matrix").removeClass("filter-loading");var c=$(b).find(".breadcrumb_text").html();$(".breadcrumb_text").html(c);c=$(b).find(".paginate").html();$(".paginate").html(c);c=$(b).find(".sidebar").html();$(".sidebar").html(c);c=$(b).find(".paginate").html();$(".paginate").empty();$(".paginate").html(c);b=$(b).find(".collection-matrix");$(".collection-matrix").remove();b.insertBefore($(".load-more__icon"));
window.history&&window.history.pushState&&window.history.pushState("","",a);$(".sidebar__collection-filter").length&&window.collectionSidebarFilter.init();"infinite_scroll"==window.PXUTheme.theme_settings.pagination_type&&enableInfiniteScroll();window.PXUTheme.currency.show_multiple_currencies&&window.currencyConverter.convertCurrencies();window.PXUTheme.theme_settings.enable_shopify_review_comments&&SPR.$(document).ready(function(){return SPR.registerCallbacks(),SPR.initRatingHandler(),SPR.initDomEls(),
SPR.loadProducts(),SPR.loadBadges()});window.recentlyViewed.init();$(".js-product_section .product_form_options").each(function(){new Shopify.OptionSelectors($(this).data("select-id"),{product:$(this).data("product"),onVariantSelected:selectCallback,enableHistoryState:$(this).data("enable-state")})});window.productPage.init();window.productPage.runOptionSelector();$("body").off("click",".swatch-element");window.productPage.productSwatches();$(".swatch_options input").each(function(){$(this).is(":checked")&&
window.quickShop.toggleSwatchImages($(this))});$(".swatch_options label").on("click",function(){window.quickShop.toggleSwatchImages($(this))});Shopify.PaymentButton&&Shopify.PaymentButton.init()},error:function(b,c,d){console.log(b);console.log(c);console.log(d);location.replace(location.protocol+"//"+location.host+a)},dataType:"html"})},processUrl:function(a,b,c){c=c.replace(/\page=(\w+)&/,"");quickFilter.updateView(a+b+c)}};
...
$('[data-dropdown-rel="search"], [data-autocomplete-true] input').on("click touchstart",function(a){if(window.PXUTheme.media_queries.medium.matches){a.stopPropagation();a=$(this).closest("form").find('[name="type"]').val();var b=$(document).scrollTop();window.searchAutocomplete.showMobileSearch(a,b)}});$(".search-close").on("click touchstart",function(){$("body").removeClass("is-active");$(".dropdown_link").removeClass("active_link");$(".dropdown_container").hide();$(".mobile_nav").find("div").removeClass("open");
...
window.PXUTheme.routes.root_url+e.url));if(e.price){function h(k){return'<span class="money">'+Shopify.formatMoney(k,$("body").data("money-format"))+"</span>"}var g=!0===e.available?e.compare_at_price_max>e.price_max||e.compare_at_price_min>e.price_min?h(e.price)+' <span class="was_price">'+h(e.compare_at_price_max)+"</span>":0<e.price?e.price_min!=e.price_max?window.PXUTheme.translation.from_text+" "+h(e.price):h(e.price):window.PXUTheme.theme_settings.free_text:window.PXUTheme.theme_settings.sold_out_text;
e.image&&f.append('<div class="thumbnail"><img class="lazyload transition--'+window.PXUTheme.theme_settings.image_loading_style+'" src="'+window.utils.addImageDimension(e.image,"_300x")+'" /></div>');f.append('<div class="search-title">'+e.title+'<br><span class="item-pricing price">'+g+"</span></div>")}else e.summary_html?("NULL"!=e.image&&f.append('<div class="thumbnail"><img class="lazyload transition--'+window.PXUTheme.theme_settings.image_loading_style+'" src="'+window.utils.addImageDimension(e.image,
"_300x")+'" /></div>'),f.append('<div class="search-title">'+e.title+'<br><span class="item-description">'+e.summary_html.replace(/(<([^>]+)>)/ig,"").slice(0,25)+" </span></div>")):e.published_at&&f.append('<div class="search-title">'+e.title+'<br><span class="item-description">'+e.body.replace(/(<([^>]+)>)/ig,"").slice(0,25)+" </span></div>");f.wrap('<li class="item-result"></li>');d.append(f.parent())}),d.prepend('<li class="all-results"><span class="search-title see-all"><a href="'+this.vars.searchPath+
"?type="+b+"&q="+this.vars.term+'*"><span>'+window.PXUTheme.translation.all_results+'</span><span class="icon-right-arrow"></span></a></span></li>'),(window.PXUTheme.currency.show_multiple_currencies||window.PXUTheme.currency.native_multi_currency)&&window.currencyConverter.init(),d.parents(".search__results-wrapper").addClass("results-found")):(d.append('<li class="item-result"><span class="no-results search-title">'+window.PXUTheme.translation.no_results+"</span></li>"),d.parents(".search__results-wrapper").removeClass("results-found"));
d.show()},showMobileSearch:function(a,b){$("body").css("max-height",window.innerHeight);$(".mobile-search").fadeIn(200);/iPad|iPhone|iPod/.test(navigator.platform)||"MacIntel"===navigator.platform&&1<navigator.maxTouchPoints?$(".mobile-search input#q").focus():setTimeout(function(){$(".mobile-search input#q").focus()},205);document.body.style.position="fixed";document.body.style.top="-"+b+"px";$(".mobile-search").css("top",b);var c=window.innerHeight-60;$(".mobile-search .search__results-wrapper").css("max-height",
c);a?$('.mobile-search [name="type"]').val(a):$('.mobile-search [name="type"]').val(window.PXUTheme.theme_settings.search_option);$(".search-form .close-search").on("click touchstart",function(d){d.preventDefault();d.stopPropagation();window.searchAutocomplete.hideMobileSearch(b);$("[data-autocomplete-true] .search__results-wrapper").hide().removeClass("results-found")});$(".search-form .submit-search").on("click touchstart",function(d){$(this).parents("form").submit()})},hideMobileSearch:function(a){$("body").css("max-height",
"none");document.body.style.position="";document.body.style.top="";window.scrollTo(0,a);$(".mobile-search").fadeOut(200);$('.mobile-search [name="q"]').val("");$("body").off("focus",".search-form .close-search");$("body").off("focus",".search-form .submit-search")},unload:function(){$("body").off("focus","[data-autocomplete-true] input");$('input[name="q"]').off();$('[data-dropdown-rel="search"], [data-autocomplete-true] input').off();$(".search__results-wrapper").remove()}};
...
window.sidebarAccordions={init:function(){var a=document.querySelector("[data-sidebar]");if(a){var b=a.querySelectorAll("[data-sidebar-block]"),c=a.querySelector("[data-faceted-filter-form]"),d="true"===a.dataset.sidebarAllowToggle,f=()=>b.forEach(h=>{h.open=!1}),e=()=>b.forEach(h=>{h.open=!0}),g=!1;a=()=>{window.PXUTheme.media_queries.medium.matches?(g||f(),g=!0):(d||e(),g=!1)};a();window.addEventListener("resize",a);c&&c.addEventListener("change",h=>{"number"!=h.target.type&&c.submit()});b.forEach(h=>
{const k=h.querySelector("summary");if(k){var m=window.animations.transition({el:h,state:h.open?"open":"closed",stateAttribute:"data-sidebar-block-state",stateChangeAttribute:"data-sidebar-block-animation"});k.addEventListener("click",l=>{l.preventDefault();if(window.PXUTheme.media_queries.medium.matches||d)({height:l}=k.getBoundingClientRect()),h.style.setProperty("--closed-height",`${l}px`),h.open?(h.style.setProperty("--open-height",`${h.scrollHeight}px`),m.animateTo("closed").then(()=>{h.open=
!1})):(h.open=!0,m.animateTo("open",{onStart:()=>{h.style.setProperty("--open-height",`${h.scrollHeight}px`)}}))})}})}}};
...
$(this).toggleClass("active");$(this).parent().next().slideToggle()})},initializeTabs:function(){$("ul.tabs > li > a").attr("data-no-instant",!0);$("body").on("click","ul.tabs > li > a",function(a){a.preventDefault();a=$(this).attr("href");"#"==a.charAt(0)&&($("ul.tabs > li > a.active").removeClass("active"),$(this).addClass("active"),$(this).parents("ul.tabs").next().find(a).show().css({display:"block"}).addClass("active").siblings().hide().removeClass("active"))})},productReviewScroll:function(){if($("#shopify-product-reviews").length&&
$(".shopify-product-reviews-badge").length)$(".spr-badge-container").on("click",function(){window.utils.scrollToTop("#shopify-product-reviews")})},scrollToTop:function(a,b){void 0!=b?$("html, body").animate({scrollTop:$(a).offset().top-b},1E3):$("html, body").animate({scrollTop:$(a).offset().top},1E3)},pageBannerCheck:function(){0< !$(".page-banner").length||$(".header").hasClass("header-background--solid")?($(".feature_image").removeClass("feature_image"),$(".header.is-absolute").removeClass("is-absolute"),
$(".secondary_logo--true").find(".secondary_logo").css("display","none"),$(".secondary_logo--true").find(".primary_logo").css("display","block")):$(".page-banner").hasClass("full-width--true")?($(".header").parent().addClass("feature_image"),$(".header").addClass("is-absolute"),$("header.feature_image").hasClass("secondary_logo--true")&&($(".secondary_logo--true").find(".secondary_logo").css("display","block"),$(".secondary_logo--true").find(".primary_logo").css("display","none"))):($(".header").parent().removeClass("feature_image"),
...
($(".secondary_logo--true").find(".secondary_logo").show(),$(".secondary_logo--true").find(".primary_logo").hide())):($('[class^="page-details"] .feature_image').removeClass("feature_image"),$('[class^="page-details"] .header.is-absolute').removeClass("is-absolute"),$("header.feature_image").hasClass("secondary_logo--true")||($(".secondary_logo--true").find(".secondary_logo").hide(),$(".secondary_logo--true").find(".primary_logo").show()))},enableDisclosure:function(){function a(e){e.siblings(".disclosure__list-wrap").is(":off-right")&&
...
{window.productPage.init();window.productPage.runOptionSelector();videoFeature.setupVideoPlayer();productMedia.setupMedia();utils.initializeTabs();productPage.productSwatches();Shopify.PaymentButton&&Shopify.PaymentButton.init()},afterShow:(d,f)=>{({src:d}=f);d=$(d).find(".quick-shop");d.addClass("quick-shop--loaded");d.addClass("content-loaded")},beforeClose:(d,f)=>{({src:d}=f);d=$(d).find(".quick-shop");d.removeClass("quick-shop--loaded");d.removeClass("content-loaded")}}))}).catch(c=>console.error(c))});
$(".swatch_options input").each(function(){$(this).is(":checked")&&window.quickShop.toggleSwatchImages($(this))});$(".swatch_options label").on("click",function(){window.quickShop.toggleSwatchImages($(this))})},toggleSwatchImages:function(a){var b=$(a).data("image");a=$(a).parents(".thumbnail").find(".image__container img");a.attr("src",b);a.attr("srcset",b)}};
