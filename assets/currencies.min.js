...
function formatWithDelimiters(a){var b=1<arguments.length&&void 0!==arguments[1]?arguments[1]:2,c=2<arguments.length&&void 0!==arguments[2]?arguments[2]:",",d=3<arguments.length&&void 0!==arguments[3]?arguments[3]:".";if(isNaN(a)||!a)return 0;b=(a/100).toFixed(b).split(c);return b[0].replace(/(\d)(?=(\d\d\d)+(?!\d))/g,"$1".concat(c))+(b[1]?d+b[1]:"")}
function formatMoney(a,b){"string"===typeof a&&(a=a.replace(".",""));var c="",d=/\{\{\s*(\w+)\s*\}\}/;switch(b.match(d)[1]){case "amount":c=formatWithDelimiters(a,2,",",".");break;case "amount_with_space_separator":c=formatWithDelimiters(a,2," ",".");break;case "amount_with_comma_separator":c=formatWithDelimiters(a,2,".",",");break;case "amount_with_apostrophe_separator":c=formatWithDelimiters(a,2,"'",".");break;case "amount_no_decimals":c=formatWithDelimiters(a,0,",",".");break;case "amount_no_decimals_with_space_separator":c=
formatWithDelimiters(a,0," ",".");break;case "amount_no_decimals_with_comma_separator":c=formatWithDelimiters(a,0,",",".")}return b.replace(d,c)}function getCentsValue(a,b,c){return-1!==b.indexOf("amount_no_decimals")?100*a:"JOD"===c||"KWD"===c||"BHD"===c?a/10:a}function getMoneyValue(a){a=a.getAttribute("data-currency-original")||a.textContent;a=parseInt(a.replace(/[^0-9]/g,""),10);return isNaN(a)?"":a}
window.currencyConverter={init:function(a){this.options=this.defaults={switcherSelector:"[data-currency-converter] .disclosure__button",priceSelector:"span.money",shopCurrency:window.PXUTheme.currency.shop_currency,defaultCurrency:window.PXUTheme.currency.default_currency,displayFormat:window.PXUTheme.currency.display_format,moneyFormat:window.PXUTheme.currency.money_format,moneyFormatNoCurrency:window.PXUTheme.currency.money_format_no_currency,moneyFormatCurrency:window.PXUTheme.currency.money_format_currency};
this.moneyFormats=moneyFormats;this.storage="currency";this.currentCurrency=null;this.isInitialised=!1;window.Currency&&!this.isInitialised&&($(this.options.switcherSelector).on("click",function(b){b.preventDefault();b.stopPropagation();b=$(this);window.currencyConverter.setCurrency(b.val())}),this.currentCurrency=this._getStoredCurrency()||this.options.defaultCurrency,this.moneyFormats[this.options.shopCurrency].money_with_currency_format=this.options.moneyFormatCurrency,this.moneyFormats[this.options.shopCurrency].money_format=
this.options.moneyFormatNoCurrency,this.isInitialised=!0,this._current())},setCurrency:function(a,b){window.currencyConverter.isInitialised&&window.currencyConverter._convertAll(a)},update:function(a){if(window.currencyConverter.isInitialised){for(var b=a.attributes,c=0;c<b.length;c++){var d=b[c];0===d.name.indexOf("data-currency")&&a.setAttribute(d.name,"")}window.currencyConverter._convertEl(a,window.currencyConverter.currentCurrency)}},_getStoredCurrency:function(){try{return localStorage.getItem(window.currencyConverter.storage)}catch(a){return console.warn(a),
window.currencyConverter.options.defaultCurrency}},_setStoredCurrency:function(a){try{localStorage.setItem(window.currencyConverter.storage,a)}catch(b){console.warn(b)}},_current:function(){for(var a=document.querySelectorAll(window.currencyConverter.options.switcherSelector),b=0;b<a.length;b+=1)for(var c=a[b].querySelectorAll("option"),d=0;d<c.length;d+=1){var e=c[d];e.selected&&e.value!==window.currencyConverter.currentCurrency&&(e.selected=!1);e.value===window.currencyConverter.currentCurrency&&
(e.selected=!0)}window.currencyConverter._convertAll(window.currencyConverter.currentCurrency)},_convertEl:function(a,b){var c=window.currencyConverter.options.shopCurrency;if(a.getAttribute("data-currency")!==b){if(a.getAttribute("data-currency-".concat(b)))a.innerHTML=a.getAttribute("data-currency-".concat(b));else{var d=window.currencyConverter.moneyFormats[c][window.currencyConverter.options.displayFormat],e=window.currencyConverter.moneyFormats[b][window.currencyConverter.options.displayFormat],
f=getMoneyValue(a),g=getCentsValue(f,d,c);if(""===g)return;f=window.Currency.convert(g,c,b);d=formatMoney(g,d);e=formatMoney(f,e);a.getAttribute("data-currency-original")||a.setAttribute("data-currency-original",d);a.setAttribute("data-currency-".concat(c),d);a.setAttribute("data-currency-".concat(b),e);a.innerHTML=e}a.setAttribute("data-currency",b)}},_convertAll:function(a){var b=window.PXUTheme.currency.show_multiple_currencies?document.querySelectorAll(window.currencyConverter.options.priceSelector):
document.querySelectorAll("#wrapper-response span.money, #estimated-shipping span.money");if(b){window.currencyConverter.currentCurrency=a;$(".currency-code").text(window.currencyConverter.currentCurrency);window.currencyConverter._setStoredCurrency(a);for(var c=0;c<b.length;c+=1)window.currencyConverter._convertEl(b[c],a)}},convertCurrencies:function(){var a=Currency.show_multiple_currencies?$(".currency-code").first().text():$("#CurrencySelector").first().val();a&&window.currencyConverter.setCurrency(a)}};
